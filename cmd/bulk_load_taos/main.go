...
import (
	...
	taos_http "github.com/influxdata/influxdb-comparisons/cmd/bulk_load_taos/http"
	"io"
	"log"
	"net/http"
	"net/url"
...
type workerConfig struct {
	...
	writer         *taos_http.HTTPWriter
	backingOffSecs float64
}

func (l *TaosBulkLoad) Init() {
	flag.StringVar(&l.url, "url", "http://localhost:6041", "TDengine URLs, comma-separated. Will be used in a round-robin fashion.")
	...
	flag.BoolVar(&l.useGzip, "gzip", false, "Whether to gzip encode requests (default false).")
	flag.IntVar(&l.ingestRateLimit, "ingest-rate-limit", -1, "Ingest rate limit in values/s (-1 = no limit).")
}

func (l *TaosBulkLoad) Validate() {
	if len(l.url) == 0 {
...
func (l *TaosBulkLoad) CreateDb() {
	// todo this fun must modify
	// this also test db connection
	err := createDb(l.url, bulk_load.Runner.DbName)
	if err != nil {
		log.Fatal(err)
	}
...
func (l *TaosBulkLoad) GetBatchProcessor() bulk_load.BatchProcessor {
	return l
}

func (l *TaosBulkLoad) GetScanner() bulk_load.Scanner {
	return l
}

func (l *TaosBulkLoad) PrepareWorkers() {
...
func (l *TaosBulkLoad) EmptyBatchChanel() {
	for range l.batchChan {
		//read out remaining batches
	}
}

func (l *TaosBulkLoad) SyncEnd() {
	<-l.inputDone
	close(l.batchChan)
}

func (l *TaosBulkLoad) CleanUp() {
...
func (l *TaosBulkLoad) PrepareProcess(i int) {
	l.configs[i] = &workerConfig{
		url:            l.url,
		backingOffChan: make(chan bool, 100),
		backingOffDone: make(chan struct{}),
	}
	l.configs[i].writer = taos_http.NewHTTPWriter(taos_http.HTTPWriterConfig{
...
func (l *TaosBulkLoad) RunProcess(i int, waitGroup *sync.WaitGroup, telemetryPoints chan *report.Point, reportTags [][2]string) error {
...
func (l *TaosBulkLoad) AfterRunProcess(i int) {
	l.configs[i].backingOffSecs = processBackoffMessages(i, l.configs[i].backingOffChan, l.configs[i].backingOffDone)
}

func (l *TaosBulkLoad) UpdateReport(params *report.LoadReportParams) (reportTags [][2]string, extraVals []report.ExtraVal) {
	...
	params.DestinationUrl = l.url
	params.IsGzip = l.useGzip

	return
}

func (l *TaosBulkLoad) IsScanFinished() bool {
	return l.scanFinished
}

func (l *TaosBulkLoad) GetReadStatistics() (itemsRead, bytesRead, valuesRead int64) {
...
func (l *TaosBulkLoad) RunScanner(r io.Reader, syncChanDone chan int) {
...
func (l *TaosBulkLoad) processBatches(w *taos_http.HTTPWriter, backoffSrc chan bool, telemetrySink chan *report.Point, telemetryWorkerLabel string, workersGroup *sync.WaitGroup, reportTags [][2]string) error {
...
func createDb(daemonUrl, dbname string) error {
	u, err := url.Parse(daemonUrl+"/rest/sql")
	if err != nil {
		return err
	}
	body := strings.NewReader(fmt.Sprintf("CREATE DATABASE %s",dbname))
	...
	if resp.StatusCode != http.StatusOK {
		if resp.StatusCode == http.StatusBadRequest {
			return fmt.Errorf("database already exists: %v", resp.StatusCode)
		}
...
func countFields(line string) int {
	...
	lineParts := strings.Split(line, "VALUES") // "measurement,tags fields timestamp"
	if len(lineParts) != 2 {
		log.Fatalf("invalid insert line '%s'", line)
	}
